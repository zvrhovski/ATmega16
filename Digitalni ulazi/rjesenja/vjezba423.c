...
int main(void){
	...
	while (1)
	{
		...
		if(get_pin(PINB,PB2) == 0){ // ako je pin PB2 u niskom stanju
		set_port(PORTB, PB6, 1);	// ukljuèi žutu LED diodu
		}
		else{
		set_port(PORTB, PB6, 0);	// inaèe je iskljuèi
		}

		if(get_pin(PINB,PB1) == 0){ // ako je pin PB1 u niskom stanju
		set_port(PORTB, PB5, 1);	// ukljuèi zelenu LED diodu
		}
		else{
		set_port(PORTB, PB5, 0);	// inaèe je iskljuèi
		}

		if(get_pin(PINB,PB0) == 0){ // ako je pin PB0 u niskom stanju
		set_port(PORTB, PB4, 1);	// ukljuèi bijelu LED diodu
		}
		else{
		set_port(PORTB, PB4, 0);	// inaèe je iskljuèi
		}
